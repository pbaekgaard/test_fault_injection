...
/*$
  @name = VerifyPIN_7_HB+FTL+INL+DPTC+DT+SC
  @feature = verifyPIN
  @fault-model = test-inversion
  @attack-scenario = oracle
  @countermeasure = Hardened booleans, inlined calls, Decrement PTC first,
                Double test, Fixed time loop, Step counter
...
int main()
{
    ...
    printf("[@] g_countermeasure = %i, g_authenticated = %b, g_ptc = %i\n", g_countermeasure, oracle(), g_ptc);
